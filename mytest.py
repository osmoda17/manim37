...
class Medicion(VGroup):
    ...
    def __init__(self, objeto, **kwargs):
        ...
        if self.dashed == True:
            medicion = DashedLine(ORIGIN, objeto.get_length() * RIGHT,
...
class PythagoreanProof(Scene):
    ...
    def pre_cuadrado(self):
        ...
        t1 = Polygon(lin, eid, ld, color=self.color_triangulos).set_fill(self.color_triangulos,
                                                                         self.opacidad_triangulos).set_stroke(None,
                                                                                                              self.grosor_lineas)
        t2 = Polygon(lin, eii, liz, color=self.color_triangulos).set_fill(self.color_triangulos,
                                                                          self.opacidad_triangulos).set_stroke(None,
                                                                                                               self.grosor_lineas)
        t3 = Polygon(liz, esi, ls, color=self.color_triangulos).set_fill(self.color_triangulos,
                                                                         self.opacidad_triangulos).set_stroke(None,
                                                                                                              self.grosor_lineas)
        t4 = Polygon(ld, esd, ls, color=self.color_triangulos).set_fill(self.color_triangulos,
                                                                        self.opacidad_triangulos).set_stroke(None,
                                                                                                             self.grosor_lineas)
        cuadrado_c = Polygon(*coordenadas_lados, color=self.color_cuadrado_c).set_fill(self.color_cuadrado_c,
                                                                                       self.opacidad_cuadradro_c)

        self.cuadrado_c = cuadrado_c

        med_ia = Medicion(Line(eii, lin), invertir=True, dashed=True, buff=0.5).add_tips().add_tex("a", buff=-3.7,
                                                                                                   color=WHITE)
        med_ib = Medicion(Line(lin, eid), invertir=True, dashed=True, buff=0.5).add_tips().add_tex("b", buff=-2.7,
                                                                                                   color=WHITE)
        med_izb = Medicion(Line(eii, liz), invertir=False, dashed=True, buff=0.5).add_tips().add_tex("b", buff=1,
                                                                                                     color=WHITE)
        med_iza = Medicion(Line(liz, esi), invertir=False, dashed=True, buff=0.5).add_tips().add_tex("a", buff=2,
                                                                                                     color=WHITE)
        med_iza[-1].rotate(-PI / 2)
        med_izb[-1].rotate(-PI / 2)
        mediciones_1 = VGroup(med_ia, med_ib, med_iza, med_izb)

        titulo = TextMobject("\\sc Доказательство теоремы Пифагора", color=WHITE).to_corner(UL)
    ...
    def pos_cuadrado(self):
        ...
        t1 = Polygon(lin, eid, ld, color=self.color_triangulos).set_fill(self.color_triangulos,
                                                                         self.opacidad_triangulos).set_stroke(None,
                                                                                                              self.grosor_lineas)
        t2 = Polygon(lin, centro, ld, color=self.color_triangulos).set_fill(self.color_triangulos,
                                                                            self.opacidad_triangulos).set_stroke(None,
                                                                                                                 self.grosor_lineas)
        t3 = Polygon(esi, liz, centro, color=self.color_triangulos).set_fill(self.color_triangulos,
                                                                             self.opacidad_triangulos).set_stroke(None,
                                                                                                                  self.grosor_lineas)
        t4 = Polygon(centro, ls, esi, color=self.color_triangulos).set_fill(self.color_triangulos,
                                                                            self.opacidad_triangulos).set_stroke(None,
                                                                                                                 self.grosor_lineas)
        cuadrado_a = Polygon(*[eii, liz, centro, lin], color=self.color_rect_a).set_fill(self.color_rect_a,
                                                                                         self.opacidad_cuadradro_a)
        cuadrado_b = Polygon(*[centro, ls, esd, ld], color=self.color_rect_b).set_fill(self.color_rect_b,
        ...
        med_ib = Medicion(Line(lin, eid), invertir=True, dashed=True, buff=0.5).add_tips().add_tex("b", buff=-2.7,
                                                                                                   color=WHITE)
        med_iza = Medicion(Line(eii, liz), invertir=False, dashed=True, buff=0.5).add_tips().add_tex("a", buff=1.8,
                                                                                                     color=WHITE)
        med_izb = Medicion(Line(liz, esi), invertir=False, dashed=True, buff=0.5).add_tips().add_tex("b", buff=1,
                                                                                                     color=WHITE)
        med_iza[-1].rotate(-PI / 2)
        med_izb[-1].rotate(-PI / 2)
